 ...
 * @return array
 */
function iw17_body_classes( $classes ) {
...
add_filter( 'body_class', 'iw17_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function iw17_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'iw17_pingback_header' );

/**
 * Change length of excerpt to 30
 */
function iw17_excerpt_length( $length ) {
	return 30;
}
add_filter( 'excerpt_length', 'iw17_excerpt_length', 999 );


/**
 * Limit max menu depth in admin panel to 2
 */
function iw17_limit_depth( $hook ) {
...
// add_action( 'admin_enqueue_scripts', 'iw17_limit_depth' );
 ...
 */
function iw17_render_dynamic_text_array( $string, $target ) {
 ...
 */
function iw17_parallax_background( $type = 'squares', $full = true ) {
 ...
 */
function iw17_get_the_post_classes() {
	global $post;
 ...
 */
function iw17_get_the_post_classes_string() {
	return implode( ' ', iw17_get_the_post_classes() );
}

function iw17_get_the_title() {
	global $post;
 ...
 */
function iw17_render_post_title( $title = false ) {
	global $post;

	if ( ! $title )
		$title = iw17_get_the_title();
 ...
 */
function iw17_template_nice_name( $id = null ) {
	if ( $id ) {
 ...
 */
function iw17_get_query_options( $paged = false ) {
    global $post;
 ...
 */
function iw17_localhost_reminder() {
...
add_action( 'admin_head', 'iw17_localhost_reminder' );
add_action( 'wp_head', 'iw17_localhost_reminder' );

/**
 * Floating action button
 */
function iw17_floating_action_button( $title ) {
	// Disable
	return;
 ...
 */
function iw17_disable_comment_url( $fields ) {
    unset( $fields['url'] );
    return $fields;
}
add_filter( 'comment_form_default_fields','iw17_disable_comment_url' );


function iw17_redirect_contact_form( $to, $postdata, $post_id ) {
...
add_filter( 'coblocks_form_email_to', 'iw17_redirect_contact_form', 10, 3 );


function iw17_custom_disable_author_page() {
    global $wp_query;
...
add_action('template_redirect', 'iw17_custom_disable_author_page');
